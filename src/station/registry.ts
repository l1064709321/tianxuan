...
const SEED: NeuralSystem[] = [
  ...
  { kind: "mamba", name: "Mamba", role: "线性状态流(快动力学)", stage: "phase1", enabled: true, status: "idle", roleTag: "central", justification: "对照基线: 线性状态建模 vs 纯 Transformer(同配置)", evidence: { metric: "事件语料 top-1(30k×2ep)", baseline: 49.1, result: 47.5, status: "pending", note: "SSM 内核 gradcheck=0.0000; 与 GRU 基线持平, 无缺口证据" } },
  { kind: "sparse-attention", name: "稀疏 Attention", role: "慢语义(事件级语义演化)", stage: "phase2", enabled: true, status: "idle", roleTag: "central", justification: "对照基线: 先验是否缺失注意力评估后定", evidence: { metric: "事件语料 top-1(30k×2ep)", baseline: 49.1, result: 47.8, status: "pending", note: "机制正确(gradcheck 0.0000)+gate 选择性双峰; 任务级提升证据未达成→不可 enabled" } },
  { kind: "titans", name: "Titans 记忆", role: "长期记忆 + L0 零算力直出", stage: "phase3", enabled: true, status: "idle", justification: "L0 记忆: 官方定位与长上下文/低成本命中一致", evidence: { metric: "L0 记忆命中率", baseline: 0, result: 0, status: "pending", note: "在线动量记忆已实现; 命中率待向量库召回量化" } },
