...
export class Executor {
  ...
  constructor(
    ...
    this.cascadeThreshold = options.cascadeThreshold ?? 0.6;
    this.coldThreshold = options.coldThreshold ?? 0.15;
  ...
  async execute(task: Task): Promise<TaskResult> {
    ...
    const maxDepth = units[units.length - 1].depth;
    const budget = Math.max(1, Math.min(task.budget || units.length, units.length));
    ...
    for (let used = 0; used < budget && targetDepth <= maxDepth; used++) {
      ...
      if (sys?.roleTag === "central" && this.probe.sampleCount(next.unitId) >= this.warmup && this.probe.ignitionAvg(next.unitId, 32) < this.coldThreshold) {
        this.audit.route({
          taskId: task.id,
          depth: next.depth,
